...
class State(rx.State):
    ...
    # --- FIX: Explicit Setters to prevent Deprecation Warnings ---
    ...
    async def handle_login(self):
        ...
        async with httpx.AsyncClient() as client:
            try:
                # Talking to Rust backend on port 8000
                ...
                if response.status_code == 200:
                    ...
                    # Future: Redirect to dashboard logic here
                else:
...
def index():
    return rx.hstack(
        ...
        spacing="0",
        bg="black" # Fallback color if image fails
    )
